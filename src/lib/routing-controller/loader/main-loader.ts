...
export default abstract class MainLoad {
    ...
    protected constructor(
        ...
        this.registerControllers().then(() => this.registerExpressRoutes());
    }
            ...
            try {
                for (const conPath of this.controllerPath) {
                    if ((typeof conPath) === "function") {
                        ...
                        await this.resolveControllerDecorator(classInstance);
                    } else {
    ...
    private resolveControllerDecorator(classInstance: ClassDecorator): Promise<any> {
